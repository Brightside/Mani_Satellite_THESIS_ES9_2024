...
Hard_Neg_IoU_threshold = 0.001;  % Minimum, as ==0 still gets filtered
...
%-----------------------------------------------------------------------------------------------------------------------
    ...
    window_sizes = [32 32; 64 64; 96 96; 128 128; 150 150; 200 200;320 320]; % from fixed ROI size, up to half the iamge size
...
%% -----------------------------------------------------------------------------------------------------------------------
    ...
    if ~isempty(current_gTruth)
        ...
        outputname = OutputFolder+"Val_IT_1_"+num2str(i)+".jpg";
...
% First validation
outputType = Kernel_type + "_IT1_";
NegHard_out_name = outputType + "HardNeg_featLabels_Dijkstra.mat";
Val_out_name = outputType + "val_stats_Dijkstra.mat";
save(NegHard_out_name,"all_hardNeg_features","all_hardNeg_labels","all_hardNed_bboxes");
